...
if [[ $iptable_mangle_exit_code == 0 ]]; then
	...
	# setup route for SABnzbd webui using set-mark to route traffic for port 8080 and 8443 to "${docker_interface}"
	echo "8080    webui" >> /etc/iproute2/rt_tables
	echo "8443    webui" >> /etc/iproute2/rt_tables
...
# accept input to SABnzbd webui port
...
iptables -A INPUT -i "${docker_interface}" -p tcp --dport 8443 -j ACCEPT
iptables -A INPUT -i "${docker_interface}" -p tcp --sport 8443 -j ACCEPT
...
if [[ $iptable_mangle_exit_code == 0 ]]; then
	# accept output from SABnzbd webui port - used for external access
	...
	iptables -t mangle -A OUTPUT -p tcp --dport 8443 -j MARK --set-mark 1
	iptables -t mangle -A OUTPUT -p tcp --sport 8443 -j MARK --set-mark 1
fi

# accept output from SABnzbd webui port - used for lan access
...
iptables -A OUTPUT -o "${docker_interface}" -p tcp --dport 8443 -j ACCEPT
iptables -A OUTPUT -o "${docker_interface}" -p tcp --sport 8443 -j ACCEPT
...
exec /bin/bash /etc/sabnzbd/start.sh
